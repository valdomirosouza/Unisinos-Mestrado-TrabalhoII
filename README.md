...
- [`picoc/picoc-search-string.md`](picoc/picoc-search-string.md) — **string de busca da RSL** derivada da síntese PICOC (blocos Intervention × Context; Outcomes só como refinamento opcional) e **calibrada contra o corpus** via `export.csv`: recall **13/14 dos estudos incluídos**; os não recuperados são estudos excluídos/inelegíveis (comportamento desejável). Sintaxes para Scopus, WoS, IEEE Xplore e ACM DL.
