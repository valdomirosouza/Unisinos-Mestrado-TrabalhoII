...
- [`picoc/picoc-search-string.md`](picoc/picoc-search-string.md) — **string de busca da RSL** derivada da síntese PICOC (blocos Intervention × Context; Outcomes só como refinamento opcional), **calibrada contra o corpus** via `export.csv` e **validada externamente na OpenAlex** (execução real por DOI): recall **13/14 dos estudos incluídos** nas duas validações; os não recuperados são estudos excluídos/inelegíveis (comportamento desejável); volume ≈ 49,7 mil (2020+). Sintaxes para Scopus, WoS, IEEE Xplore e ACM DL; execução no Scopus pendente de credencial da API Elsevier.
